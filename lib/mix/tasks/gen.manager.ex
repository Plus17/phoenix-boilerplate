defmodule Mix.Tasks.Gen.Manager do
  ...
  def run(args) do
    ...
    {context, schema} = build(args) |> IO.inspect(label: "BUILD")
  ...
  def build(args, help \\ __MODULE__) do
    ...
    [context_name, schema_name, plural | schema_args] = validate_args!(parsed, help) |> IO.inspect(label: "ARGS")
    schema_module = inspect(Module.concat(["Schemas", context_name, schema_name])) |> IO.inspect(label: "SCHEMA MODULE")
    opts |> IO.inspect(label: "OPTS")
    help |> IO.inspect(label: "HELP")
    ...
    context = Context.new("Contexts.#{context_name}.#{schema_name}Manager", schema, opts) |> Map.put(:test_fixtures_file, test_factory_file)
    {context, schema}
  end
  ...
  def ensure_test_fixtures_file_exists(%Context{test_fixtures_file: test_fixtures_file} = context, paths, binding) do
    test_fixtures_file |> IO.inspect(label: "test_fixtures_file")
    paths |> IO.inspect(label: "PATHS")
    binding |> IO.inspect(label: "BINDING")
    unless Context.pre_existing_test_fixtures?(context) do
      Mix.Generator.create_file(test_fixtures_file, Mix.Phoenix.eval_from(paths, "priv/templates/phx.gen.context/factory.ex", binding))
    end
  end
